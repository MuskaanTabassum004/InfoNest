...
export const ResumableFileUpload: React.FC<ResumableFileUploadProps> = ({
  ...
  const handleFileSelect = useCallback(
        ...
        const uploadId = await resumableUploadManager.addUpload(
          ...
          (result) => {
            if (result instanceof Error) {
              ...
              console.log(`✅ File uploaded: ${file.name} → ${result.url}`);
          ...
          // Pass context for organized file structure and cleanup
          articleId
            ? {
                articleId,
                type: "article",
                targetField: accept?.includes("image")
                  ? "coverImage"
                  : "attachment",
              }
            : undefined
    ...
    [userProfile, folder, onUploadComplete, onUploadError, articleId]
  );
  ...
  const handleUploadAction = useCallback(
    (uploadId: string, action: "pause" | "resume" | "cancel" | "delete") => {
      switch (action) {
        case "pause":
        ...
        case "cancel":
          ...
          break;
        case "delete":
          resumableUploadManager.deleteCompletedUpload(uploadId);
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex items-center space-x-3 text-xs text-gray-600">
                      ...
                      <span>
                        {
                          activeUploads.filter((u) =>
                            ["running", "paused"].includes(u.progress.state)
                          ).length
                        }{" "}
                        active
                      </span>
              ...
              {showManager && (
                <div className="max-h-64 overflow-y-auto">
                  {activeUploads.map((upload) => (
                    <div
                      key={upload.id}
                      className={`p-4 border-b border-gray-100 ${getUploadStatusColor(
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex items-center space-x-1 ml-2">
                          {upload.progress.state === "running" && (
                            <button
                              onClick={() =>
                                handleUploadAction(upload.id, "pause")
                              }
                          ...
                          {upload.progress.state === "paused" && (
                            <button
                              onClick={() =>
                                handleUploadAction(upload.id, "resume")
                              }
                          ...
                          {/* Show delete button for completed uploads */}
                          ...
                          {/* Show cancel button for active uploads */}
                          {["running", "paused", "error"].includes(
                            upload.progress.state
                          ) && (
                            <button
                              onClick={() =>
                                handleUploadAction(upload.id, "cancel")
                              }
                          ...
                          {/* Show delete button for failed uploads */}
                          {upload.progress.state === "error" && (
                            <button
                              onClick={() =>
                                handleUploadAction(upload.id, "delete")
                      ...
                      <div className="mb-2">
                        <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
                          ...
                          <span>
                            {resumableUploadManager.formatFileSize(
                              upload.progress.bytesTransferred
                            )}{" "}
                            /{" "}
                            {resumableUploadManager.formatFileSize(
                              upload.progress.totalBytes
                            )}
                          </span>
                        </div>
                      ...
                      {upload.progress.state === "running" && (
                        <div className="flex items-center justify-between text-xs text-gray-500">
                          <div className="flex items-center space-x-1">
                            ...
                            <span>
                              {resumableUploadManager.formatSpeed(
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <span>
                              {resumableUploadManager.formatTime(
          ...
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {isOnline
                ...
                : "Offline - Files will upload when online"}
            </h3>
            <p className="text-sm text-gray-600">
              Supports resumable uploads with automatic retry on network
              reconnection
            </p>
