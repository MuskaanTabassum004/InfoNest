...
export const Layout: React.FC = () => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen" style={{ backgroundColor: '#EFEDFA' }}>
        <Outlet /> {/* ✅ Still render nested routes for unauthenticated pages */}
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-6">
              ...
              <Link
                to="/search"
                className={getLinkClass('/search', 'blue')}
              >
              ...
              {isAdmin && (
                <Link
                  to="/admin"
                  className={getLinkClass('/admin', 'amber')}
                >
  ...
  function getLinkClass(path: string, color: 'blue' | 'purple' | 'amber') {
    ...
    const base = 'flex items-center space-x-2 px-3 py-2 rounded-lg transition-colors';
